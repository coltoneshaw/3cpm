...
import { Type_ActiveDeals, Type_MarketOrders } from "@/types/3Commas";
...
const calcNew = ({ addFunds, atPrice, tpPercent }: calc, ordersData: Type_MarketOrders[], currentPrice: number, bought_volume: number) => {
...
const calcOriginal = ({ bought_average_price, take_profit, current_price, bought_volume }: Type_ActiveDeals) => {
...
function DCA({ row, ordersData }: DCA) {
                    ...
                    {calcObject.map(row => {
                        return (
                            <tr key={row.name}>
                                ...
                                <td className=" monospace-cell">{parseNumber(row.tpPercent, 2)}%</td>
                                <td className=" monospace-cell">{parseNumber(row.gainRequired, 2)}%</td>
