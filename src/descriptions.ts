...
const descriptions = {
    calculations: {
        ...
        risk: (maxDCA:number, totalBankroll:number, inactiveBotFunds:number) => ` is calculated by dividing your total DCA max risk of ${parseNumber(maxDCA + inactiveBotFunds)} by your current bankroll of ${parseNumber(totalBankroll)}. ${(inactiveBotFunds) ? 'This includes your bots that have the ability to open additional deals worth ' + parseNumber(inactiveBotFunds) +'.': ''}`,
