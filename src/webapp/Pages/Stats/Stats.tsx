...
import { ButtonElements } from './types';
      ...
      <div className="flex-row statHeaderRow">
        <div className="flex-row menuButtons">
          ...
          <div>
            <ButtonGroup aria-label="outlined primary button group" disableElevation disableRipple>
              {
                buttonElements.map((button) => {
                  if (button.id === currentView) {
                    return (
                      <Button
                        key={button.id}
                        onClick={() => viewChanger(button.id)}
                  ...
                  return (
                    <Button
                      className="secondaryButton-outline"
                      key={button.id}
                      onClick={() => viewChanger(button.id)}
                    >
                      {button.name}
            ...
            <UpdateDataButton
              key="updateDataButton"
              className="CtaButton"
              style={{
                margin: 'auto', height: '36px', marginLeft: '15px', padding: '5px 15px',
              }}
            />
          </div>
        </div>
