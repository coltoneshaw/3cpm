...
async function bots(profileData: Type_Profile) {
  ...
  for (let offset = 0; offset < offsetMax; offset += perOffset) {
    response = await api.getBots({
      limit: 100, sort_by: 'updated_at', sort_direction: 'desc', offset,
    });
...
async function getDealsUpdate(perSyncOffset: number, type: string, profileData: Type_Profile) {
  ...
  return { deals: [...deals, ...activeDeals], lastSyncTime };
}
...
async function getDealsThatAreUpdated(api: threeCommasAPI, perSyncOffset: number, { id, lastSyncTime }: { id: string, lastSyncTime: number | null }) {
  ...
  for (let offset = 0; offset < offsetMax; offset += perOffset) {
    ...
    response = await api.getDeals({
      limit: perOffset, order: 'updated_at', order_direction: 'desc', offset, scope: 'active, completed, finished',
    });
    if (response.length > 0) { responseArray.push(...response); }
...
async function deals(offset: number, type: string, profileData: Type_Profile) {
  const { deals, lastSyncTime } = await getDealsUpdate(offset, type, profileData);
  const dealArray = [];
  ...
  for (const deal of deals) {
    ...
    const tempObject = {
      ...
      max_deal_funds: (activeDeal) ? calcDealMaxFunds(+bought_volume, +base_order_volume, +safety_order_volume, +max_safety_orders, completed_safety_orders_count, +martingale_volume_coefficient, market_order_data.active) : null,
...
async function getAccountDetail(profileData: Type_Profile) {
  ...
  for (const account of accountData.filter((a: any) => accountIDs.includes(a.id))) {
